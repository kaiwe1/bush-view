...
| 召唤师查询 | 对局详情 |
| --------- | ------- |
| ![搜索](docs/search.gif) | ![资料](docs/profile.gif) |
...
- Node.js 和 npm
- 英雄联盟客户端已启动并登录
- 建议用管理员权限启动终端，否则开发模式下也可能无法读取 LCU 进程参数
...
- `npm run publish`：发布到 GitHub，配置见 `forge.config.ts`
...
5. `src/preload/index.ts` 通过 `contextBridge` 暴露 `window.electronAPI`
6. React 组件通过 `window.electronAPI` 获取数据
